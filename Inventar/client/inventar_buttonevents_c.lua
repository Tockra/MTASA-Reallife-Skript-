...
function showInfo(name,info,x,y,tx,ty,obx,oby,bx,by)
		...
		dxDrawImage(x,y,bx + 13 - tx,by + 31 - ty,"inventar/images/infoblubf.png",0.0,0.0,0.0,tocolor(255,255,255,InventarBlipAlpha),true)
...
local function onItemClick(itemname,itemid,tasche,platz)
	if(tasche == "Waffen") then
		if(not lockItemUseState[tasche] or not lockItemUseState[tasche][platz]) then
			setItemsRGBDefault(tasche)
			setItemRGB(platz,0,255,0)
			lockItemUseState[tasche] = {[platz]=true}
			triggerServerEvent("onPlayerWeaponUse",getLocalPlayer(),itemid)
		else
			setItemsRGBDefault(tasche)
			setItemRGB(platz,255,255,0)
			triggerServerEvent("onPlayerWeaponUse",getLocalPlayer(),itemid,true)
		end
	end
end
...
local function onWeaponFire(weaponid,ammoLeft)
	if(ammoLeft == 0) then
		setItemsRGBDefault("Waffen")
	end
	triggerServerEvent("onWeaponFire",getLocalPlayer(),weaponid,ammoLeft)
end
addEventHandler("onClientPlayerWeaponFire",getLocalPlayer(),onWeaponFire)