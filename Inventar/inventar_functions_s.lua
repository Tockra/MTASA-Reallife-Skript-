...
function removeItem(player,tasche,platz,anzahl)
	...
	if(tasche == "Waffen" and getElementData(player,"Item",id.."_Menge") - anzahl == 0) then
...
function giveItem(player,item,anzahl,tasche,platz)
	...
	if(anzahl <= 0 or anzahl > getCountOfPlaces(player,tasche,item) ) then
		return false
	end
	if(platz) then
		local id = getElementData(player,"Item_"..tasche,platz.."_id")
		if(id and getElementData(player,"Item",id) == item) then
			if(anzahl > maxLen) then
				local wert = maxLen - getElementData(player,"Item",id.."_Menge")
				giveItem(player,item,wert,tasche,platz)
				anzahl = anzahl - wert
				while(anzahl > maxLen) do
					giveItem(player,item,maxLen,tasche,platz)
					anzahl = anzahl - maxLen
					platz = getLowEmptyPlace(player,tasche)
				end
			elseif(anzahl + getElementData(player,"Item",id.."_Menge") > maxLen and getElementData(player,"Item",id.."_Menge") ~= maxLen) then
				local full = maxLen - getElementData(player,"Item",id.."_Menge")
				local rest = anzahl - full
				if(full > 0) then
	...
	if(not platz) then
		...
		if(platz) then
			...
			elseif(getElementData(player,"Item",id.."_Menge") + anzahl > maxLen and getElementData(player,"Item",id.."_Menge") ~= maxLen) then
				local full = maxLen - getElementData(player,"Item",id.."_Menge")
				local rest = anzahl - full
				if(full > 0) then
					giveItem(player,item,full,tasche,platz)
